...
export const ActiveJobScreen = () => {
  ...
  const getNextStatus = () => {
    switch (activeJob.status) {
      case 'ACCEPTED': return { status: 'ARRIVED', label: 'Mark as Arrived' };
      case 'ARRIVED': return { status: 'IN_PROGRESS', label: 'Start Service' };
      case 'IN_PROGRESS': return { status: 'COMPLETED', label: 'Complete Job' };
      default: return null;
    }
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { borderBottomColor: colors.border, backgroundColor: colors.card }]}>
        ...
        <View style={[styles.statusBadge, { backgroundColor: colors.primary + '20' }]}>
          <Text style={[styles.statusText, { color: colors.primary }]}>{activeJob.status.replace('_', ' ')}</Text>
        </View>
      </View>
      ...
      <View style={styles.mapContainer}>
        <MockMap 
          ...
          providerLocation={{ latitude: activeJob.customerLocation?.latitude || 0, longitude: activeJob.customerLocation?.longitude || 0 }} 
        />
      </View>

      <ScrollView style={styles.detailsContainer}>
        <Card style={styles.card}>
          <Text style={[styles.sectionTitle, { color: colors.textMuted }]}>CUSTOMER DETAILS</Text>
          <View style={styles.customerRow}>
            ...
            <View style={styles.customerInfo}>
              ...
              <Text style={[styles.customerPhone, { color: colors.textMuted }]}>Contact via App</Text>
            </View>
        ...
        <Card style={styles.card}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={[styles.detailText, { color: colors.text }]}>{activeJob.serviceType}</Text>
          </View>
          <View style={styles.detailRow}>
            <Ionicons name="location-outline" size={20} color={colors.textMuted} />
            <Text style={[styles.detailText, { color: colors.text }]}>{activeJob.customerLocation?.address || 'Unknown Location'}</Text>
          </View>
          <View style={styles.detailRow}>
            <Ionicons name="cash-outline" size={20} color={colors.textMuted} />
            <Text style={[styles.detailText, { color: colors.text }]}>Estimated: ${activeJob.estimatedPrice}</Text>
          </View>
          {activeJob.notes && (
      ...
      <View style={[styles.footer, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
        {nextAction ? (
          ...
          <Text style={[styles.waitingText, { color: colors.textMuted }]}>Waiting for customer payment...</Text>
        )}
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  notesContainer: { marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: '#E5E7EB' },
