...
export const LiveTrackingScreen = () => {
	...
	// Local copy of the customer's GPS — used directly by LiveMap
	...
	const handleCancelRequest = () => {
		...
		if (driverAssigned) {
			Alert.alert(
				"Cancel Service?",
				"A $5.00 cancellation fee applies since a driver has already been assigned.",
				[
	...
	const handleMarkComplete = () => {
		Alert.alert(
			"Mark Job Complete?",
			"Confirm the service has been fully completed. This will process your payment.",
			[
				{ text: "Not Yet", style: "cancel" },
				{
					...
					onPress: async () => {
						...
						try {
							await api.put(`/jobs/${job.id}/complete`);
							// job-completed socket event will fire and transition to payment
						} catch (err: any) {
		...
		return (
			<SafeAreaView style={[styles.searchingContainer, { backgroundColor: colors.background }]}>
				<View style={styles.searchingContent}>
					...
					<Text style={[styles.searchingTitle, { color: colors.text }]}>No Drivers Available</Text>
					<Text style={[styles.searchingDesc, { color: colors.textMuted }]}>
						We couldn't find a nearby provider right now. Please try again in a few minutes or contact support.
					</Text>
				</View>
				...
				<View style={styles.searchingContent}>
					...
					{searchMessage ? (
						<View style={[styles.radiusBadge, { backgroundColor: colors.primary + "18" }]}>
							...
							<Text style={[styles.radiusText, { color: colors.primary }]}>{searchMessage}</Text>
						</View>
					) : (
						<Text style={[styles.searchingDesc, { color: colors.textMuted }]}>
					...
					{isExpanding && displayPrice != null && (
						<View style={[styles.priceRow, { borderColor: colors.border }]}>
							<Text style={[styles.priceLabel, { color: colors.textMuted }]}>Updated estimate</Text>
	...
	// eta === 0  → driver is basically here  (< 1 min by Haversine)
	// eta > 0    → show "N min"
	// eta null   → not yet calculated
	const etaDisplay = eta === 0 ? "Arriving" : eta != null ? `${eta} min` : "--";
	...
	const canMarkComplete = job.status === "arrived" || job.status === "in_progress";
	...
	return (
		<View style={styles.container}>
			...
			<View style={[styles.bottomSheet, { backgroundColor: colors.card }]}>
				...
				{/* Mark Complete — shown once driver has arrived */}
				{canMarkComplete && (
					<PrimaryButton
...
const styles = StyleSheet.create({
	...
	radiusBadge: {
		...
		borderRadius: 20, marginTop: 16, marginBottom: 4,
	},
	...
	safeTop: {
		...
		zIndex: 10, paddingHorizontal: 16, paddingTop: 16,
	},
	topBanner: {
	...
	avatar: {
		...
		backgroundColor: "#E5E7EB", borderWidth: 3,
